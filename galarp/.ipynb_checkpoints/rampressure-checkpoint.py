...
class RPSim:
    ...
    def run(self, particles,
        ...
        if printout:
            ...
            print("".center(printout_width, "-"))
            print(f' Container contains {len(particles)} particles '.center(printout_width, "-"))
        ...
        if debug:
            for p in tqdm(particles):
                orbit = integrator.run(gd.combine(particles), dt=dt, t1=0, t2=integration_time)
            return
            
        orbits = integrator.run(gd.combine(particles), dt=dt, t1=0, t2=integration_time)
